...
{
    public partial class 周防灘InputForm : Form
    {
        ...
        private void 周防灘InputForm_Load(object sender, EventArgs e)
        {
            set利用区分();
            setNO();
            setドアNO();
            ...
            {//変更の場合
                ...
                while (rs.Read())
                {
                    ...
                    this.txNo.Text = DB.NVL(rs["車番"]).ToString();
        ...
        private void setドアNO()
        {
            ...
            String query = $"SELECT DISTINCT `車番`,CONCAT(`陸運支局名`,`分類番号`,`ひらがな`,`ナンバー`) as ＮＯ FROM common_db.`l_m_車番マスタ` WHERE CONCAT(`陸運支局名`,`分類番号`,`ひらがな`,`ナンバー`) LIKE '%{this.txNo.Text}%'";
        ...
        private void setNO()
        {
            ...
            String query = $"SELECT DISTINCT `車番`,CONCAT(`陸運支局名`,`分類番号`,`ひらがな`,`ナンバー`) as ＮＯ FROM common_db.`l_m_車番マスタ` WHERE CONCAT(`陸運支局名`,`分類番号`,`ひらがな`,`ナンバー`) LIKE '%{this.txNo検索用.Text}'";

            MySqlDataReader rs;
            rs = DB.GetReader(myconn, query);

            this.txNo.Items.Clear();
            var _ShipInfo = new ShipInfo();
            _ShipInfo.item1 = "";
            this.txNo.Items.Add(_ShipInfo);
            while (rs.Read())
            {
                _ShipInfo = new ShipInfo();
        ...
        private void bt入力_Click(object sender, EventArgs e)
        {
            ...
            _ShipInfo.車番 = this.txNo.Text;
            _ShipInfo.ドアNO = this.txドアNO.Text;
            _ShipInfo.車長 = this.tx車長.Text;
            _ShipInfo.請求額 = Func.Nz(this.tx請求額.Text);
            _ShipInfo.調整金 = this.current調整金;
            //_ShipInfo.消費税 = "0";
        ...
        private void txドアNO_SelectedValueChanged(object sender, EventArgs e)
        {
        }

        private void txドアNO_TextChanged(object sender, EventArgs e)
        {
        }

        private void txドアNO_SelectionChangeCommitted(object sender, EventArgs e)
        {
        }
        ...
        private void txNo_SelectedIndexChanged(object sender, EventArgs e)
        {
            setドアNO();
        }

        private void txNo検索用_TextChanged(object sender, EventArgs e)
        {
            setNO();
        }

        private void txNo_Enter(object sender, EventArgs e)
        {
            txNo.DroppedDown = true;
        }

        private void tx請求額_TextChanged(object sender, EventArgs e)
        {
            _ShipInfo.消費税 = (Int32.Parse(Func.Nz(this.tx請求額.Text)) * 0.1).ToString();
        }
    }
}
