...
{
    public partial class 川崎近海Form : Form
    {
        ...
        public string ShipName = "川崎近海";
        public 川崎近海Form()
        {
            InitializeComponent();
        }

        private void 川崎近海Form_Load(object sender, EventArgs e)
        {
            this.dgv1.Columns["利用区分"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
            this.dgv1.Columns["NO"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            this.dgv1.Columns["積日"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
            ...
            this.dgv1.Columns["SIZE"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            this.dgv1.Columns["品目"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
        ...
        private void setMainDataGrid()
        {
            ...
            //query.Append($" select t1.sid,t1.`利用区分`,t1.`乗船日`,t1.`車番`,t1.`ドアNO`,t1.`車長`,t1.`請求額`,t1.`調整金`,t1.`消費税`,t1.`計上年月` from t_予約 t1,t_調整金 t2 WHERE t1.部門CD = '{this.current部門CD}' AND t1.計上年月 like '%{this.tx計上年.Text}{this.tx計上月.Text}%' AND t1.フェリー会社 = '{ShipName}' AND t2.年月 like '%{this.tx計上年.Text}{this.tx計上月.Text}%' AND t2.フェリー名 = '{ShipName}' AND t1.車長 = t2.長さ order by 乗船日 desc ");
            query.Append($" select t1.sid,t1.`利用区分`,t1.`乗船日`,t1.`車番`,t1.`ドアNO`,t1.`車長`,t1.`請求額`,t1.`調整金`,t1.`消費税`,t1.`計上年月` from t_予約 t1 WHERE t1.部門CD = '{this.current部門CD}' AND t1.計上年月 like '%{this.tx計上年.Text}{this.tx計上月.Text}%' AND t1.フェリー会社 = '{ShipName}' order by 乗船日 desc ");
            ...
            while (i < max)
            {
                ...
                //tx燃料価格調整金.Text = Func.Nz(dv.Table.Rows[i]["調整金"].ToString());
                i++;
            }
        ...
        private void dgv1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                川崎近海InputForm nextForm = new 川崎近海InputForm();
        ...
        {
            if (this.tx計上年.Text == "" || this.tx計上月.Text == "" || this.tx燃料価格調整金.Text == "")
            {
                MessageBox.Show($"計上年/計上月/燃料価格調整金の項目を入れて、\r\n「新規作成」ボタンを押してください。");
            }
            else
            {
                ...
                {//編集データあり
                    ...
                    row["調整金"] = nextForm._ShipInfo.調整金;
                    row["消費税"] = (int.Parse(Func.Nz(nextForm._ShipInfo.請求額)) *0.1).ToString();
        ...
        private void saveData()
        {
            try
            {
                ...
                while (i < max)
                {
                    ...
                    //comm.Parameters["@sid"].Value = 12345;
                    ...
                    //comm.Parameters["@調整金"].Value = this.tx燃料価格調整金.Text;
