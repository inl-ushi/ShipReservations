...
{
    public partial class オーシャンInputForm : Form
    {
        public ShipInfo _ShipInfo = new ShipInfo();
        public string currentsid;
        public string current調整金;
        public string current計上年月;
        public オーシャンInputForm()
        {
            InitializeComponent();
        }

        private void オーシャンInputForm_Load(object sender, EventArgs e)
        {
            set利用区分();
            set車番();
            setドアNO();
            ...
            {//変更の場合
                ...
                String query = $"SELECT sid,部門CD,フェリー会社,利用区分,乗船日,車番,ドアNO,車長,作業乗船,作業下船,請求額,調整金,消費税,計上年月 FROM t_予約 t1 WHERE sid = {currentsid}";
                ...
                while (rs.Read())
                {
                    ...
                    {
                        this.dtp乗船.Text = DB.NVL(rs["乗船日"]).ToString();
                    }
                    this.tx車番.Text = DB.NVL(rs["車番"]).ToString();
                    this.txドアNO.Text = DB.NVL(rs["ドアNO"]).ToString();
                    this.tx車長.Text = DB.NVL(rs["車長"]).ToString();
                    this.tx乗船料.Text = DB.NVL(rs["作業乗船"]).ToString();
                    this.tx下船料.Text = DB.NVL(rs["作業下船"]).ToString();
                    this.tx船送料.Text = DB.NVL(rs["請求額"]).ToString();
        ...
        private void bt入力_Click(object sender, EventArgs e)
        {
            ...
            if (this.dtp乗船.Text == null || this.dtp乗船.Text == "" || this.dtp乗船.Text == " ")
            {
                _ShipInfo.乗船日 = " ";
            }
            else
            {
                _ShipInfo.乗船日 = this.dtp乗船.Value.ToString("yyyy/MM/dd");
            }
            _ShipInfo.車番 = this.tx車番.Text;
            _ShipInfo.ドアNO = this.txドアNO.Text;
            _ShipInfo.車長 = this.tx車長.Text;
            _ShipInfo.請求額 = this.tx船送料.Text;
            _ShipInfo.調整金 = this.tx調整金.Text;
            _ShipInfo.作業乗船 = this.tx乗船料.Text;
            _ShipInfo.作業下船 = this.tx下船料.Text;
            _ShipInfo.小計 = this.tx小計.Text;
            _ShipInfo.消費税 = ((Int32.Parse(this.tx船送料.Text) + Int32.Parse(this.tx調整金.Text) + Int32.Parse(this.tx乗船料.Text)+ Int32.Parse(this.tx下船料.Text))*0.1).ToString();
        ...
        private void tx車長_TextChanged(object sender, EventArgs e)
        {
            ...
            String query = $"SELECT 調整金 FROM t_調整金 t1 WHERE 年月 = {current計上年月} and フェリー名 = 'オーシャン' and 長さ = {this.tx車長.Text}";
        ...
        private void tx船送料_TextChanged(object sender, EventArgs e)
        {
        }

        private void tx乗船料_TextChanged(object sender, EventArgs e)
        {
        }

        private void tx下船料_TextChanged(object sender, EventArgs e)
        {
        }
    }
}
