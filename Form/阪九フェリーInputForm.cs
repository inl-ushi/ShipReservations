...
{
    public partial class 阪九フェリーInputForm : Form
    {
        public ShipInfo _ShipInfo = new ShipInfo();
        public string currentsid;
        public 阪九フェリーInputForm()
        {
            InitializeComponent();
        }

        private void 阪九フェリーInputForm_Load(object sender, EventArgs e)
        {
            set利用区分();
            set車番();
            setドアNO();
            ...
            {//変更の場合
                ...
                while (rs.Read())
                {
                    ...
                    {
                        this.dtp年月日.Text = DB.NVL(rs["乗船日"]).ToString();
                        this.tx曜日.Text = DateTime.Parse(DB.NVL(rs["乗船日"]).ToString()).ToString("ddd");
                    }
                    this.tx車番.Text = DB.NVL(rs["車番"]).ToString();
                    this.txドアNO.Text = DB.NVL(rs["ドアNO"]).ToString();
                    this.tx全長.Text = DB.NVL(rs["車長"]).ToString();
                    this.tx請求金額.Text = DB.NVL(rs["請求額"]).ToString();
            ...
            this.tx曜日.Text = this.dtp年月日.Value.ToString("ddd");
        }
        ...
        private void bt入力_Click(object sender, EventArgs e)
        {
            ...
            if (this.dtp年月日.Text == null || this.dtp年月日.Text == "" || this.dtp年月日.Text == " ")
            {
                _ShipInfo.乗船日 = " ";
            }
            else
            {
                _ShipInfo.乗船日 = this.dtp年月日.Value.ToString("yyyy/MM/dd");
            }
            _ShipInfo.車番 = this.tx車番.Text;
            _ShipInfo.ドアNO = this.txドアNO.Text;
            _ShipInfo.車長 = this.tx全長.Text;
            _ShipInfo.請求額 = this.tx請求金額.Text;
            _ShipInfo.調整金 = "0";
            _ShipInfo.消費税 = "0";
        ...
        private void dtp年月日_ValueChanged(object sender, EventArgs e)
        {
            this.tx曜日.Text = this.dtp年月日.Value.ToString("ddd");
        }
